...
(() => {
  ...
  // Preset colors shown as a small icon-grid submenu. Last entry opens
  // the OS picker for a custom hex.
  const SWATCH_COLORS = [
  ...
  function swatchIconDataUri(hexColor) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16">` +
      `<circle cx="8" cy="8" r="7" fill="${hexColor}" stroke="rgba(255,255,255,0.35)" stroke-width="1"/></svg>`;
    return `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`;
  }

  function customIconDataUri() {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16">` +
      `<defs><linearGradient id="g" x1="0" y1="0" x2="16" y2="16">` +
      `<stop offset="0%" stop-color="#ff6b6b"/>` +
      `<stop offset="25%" stop-color="#ffe27c"/>` +
      `<stop offset="50%" stop-color="#7cffb3"/>` +
      `<stop offset="75%" stop-color="#7c9eff"/>` +
      `<stop offset="100%" stop-color="#d67cff"/>` +
      `</linearGradient></defs>` +
      `<circle cx="8" cy="8" r="7" fill="url(#g)" stroke="rgba(255,255,255,0.35)" stroke-width="1"/></svg>`;
    return `data:image/svg+xml;utf8,${encodeURIComponent(svg)}`;
  }
  ...
  function buildColorMenu(menupopup, getPendingFolder) {
    ...
    customItem.setAttribute('tooltiptext', 'Custom…');
    customItem.setAttribute('image', customIconDataUri());
    ...
    popup.addEventListener('command', async (event) => {
      ...
      if (event.target.id === CUSTOM_SWATCH_ID) {
        const overrides = readOverrides();
